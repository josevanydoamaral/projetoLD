...
app.post("/conversation", (req, res) => {
    ...
    res.status(201).send(conversationResponses);
});
...
app.post("/conversation/:conversationId", (req, res) => {
    ...
    res.status(200).json(conversationResponses);
});
...
app.get("/conversation/:conversationId", async (req, res) => {
    ...
    queryStatus[conversationId] = "processing";
    ...
    const response = await fakeSlowResponse(conversationResponses[conversationId]);
    queryStatus[conversationId] = "completed";

    res.json(response);
});

// GET endpoint to fetch the status of conversation
app.get("/conversation/:conversationId/status", async (req, res) => {
    
    const { conversationId } = req.params

    res.json({ status: queryStatus[conversationId] || "not found" })
})
